...
async def get_room(room_name) -> RoomInDB:
    ...
    db_rooms = db_client.rooms
    row = db_rooms.find_one({"room_name": room_name})
    if row is not None:
        row = format_ids(row)
        return row
    else:
...
async def add_user_to_room(username: str, room_name: str):
    ...
    try:
        ...
        collection = db_client.rooms
        username_list = [m["username"] for m in room["members"]]
        if user.username not in username_list:
            logger.info("Adding %s to members", user.username)
            collection.update_one({"_id": ObjectId(room._id)}, {
                                  "$push": {"members": user}})
            return True
        else:
            logger.info("%s is already a member", user.username)
            return True
    except ValueError as error:
        logger.error("Error updating members: %s", error)
        return None
...
async def remove_user_from_room(user: User, room_name: str, username=None):
    ...
    try:
        ...
        logger.error("Error updating members: %s", e)
        return False
